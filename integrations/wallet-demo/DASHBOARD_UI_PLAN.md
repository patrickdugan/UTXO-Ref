...
2. Profile switch panel
   ...
   - Demo value: lets the same UI explain whether it is running against mock, a modular local testnet, or a future Bitcoin testnet backend.
...
Packaging note: `integrations/wallet-dashboard-vercel` mirrors this UI with Vercel serverless mock endpoints so the dashboard can be deployed as a standalone subrepo while the local testnet sidecar remains the source of richer live-demo data.
...
7. Wallet integration readiness checklist
   - Status: implemented.
   - Surface: tracks LDK Node, LND, Core Lightning, Bark/Ark, Taproot Assets, modular testnet, and Bitcoin testnet adapter status.
